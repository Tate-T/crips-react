...
const Catalog = ({ id, img, name, category, colors, price, discountPrice, media, style }) => {
	return (
		<NavLink to={id} className={style.catalog__item}>
			...
			{discountPrice ? (
				<p className={style["catalog__price--discount"]}>
					{parseFloat(discountPrice || 0).toFixed(2)} EUR <span className={style["catalog__price--old"]}>{parseFloat(price || 0).toFixed(2)} EUR</span>
				</p>
			) : (
				<p className={style.catalog__price}>{parseFloat(price || 0).toFixed(2)} EUR</p>
			)}
					...
					colors.map((item, index) => (
						<li className={style.colors__item} key={index}>
							<button type="button" className={style.catalog__color}>
								<div className={style.color} style={{ backgroundColor: item }} />
							</button>
						</li>
					))}
			</ul>
